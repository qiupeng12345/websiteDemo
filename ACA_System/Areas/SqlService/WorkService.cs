...
namespace ACA_System.Areas.SqlService
{
    public class WorkService
    {
        ...
        public object GetWorkCount( DateTime timeStart, DateTime timeEnd, string barcode, string recipes, string worker)
        {
            using (IDbConnection db = new SqlConnection(DbHelper.connectString))
            {
                StringBuilder sb = new StringBuilder();
        ...
        public string[] GetFilterName(int start, int limit, DateTime timeStart, DateTime timeEnd, string barcode, string recipes, string worker)
        {
            using (IDbConnection db = new SqlConnection(DbHelper.connectString))
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("select distinct  ProductModel from ProductCode where Time between '" + timeStart + "' and '" + timeEnd.ToString() + "'");
                if (recipes != "")
                {
                    sb.Append(" and ProductModel Like '%" + recipes + "%'");
                }
                ...
                //sb.Append(" order by id desc Offset " + ((start - 1) * limit).ToString() + " Row Fetch Next " + limit.ToString() + " Rows Only");
                return db.Query<string>(sb.ToString()).ToArray();
            }
        }
